...
#'
#' @examples
growthrates.distribution.tax.rank.ridges.divided <- function(data, phylum_to_explore, title,
                                                     axis_y_title, 
                                                     x_c, x_o, x_f, x_a){
  data_filt <- data %>%
  ...
  plot_class <- data_filt %>%
    ...
    ggplot(aes(y = class_f, x = slope_chosen_days, fill = class_f, label = counts))+
    # geom_histogram(aes(x=slope_chosen_days, group = class), colour = "#5F9660",  fill = "#5F9660", #adjust= 0.5,
    #                binwidth = bin_width_c,
    #                show.legend = FALSE, alpha = 0.2)+# bins = bin_width_c,
    geom_density_ridges(aes(fill = class_f), scale = 1, alpha = 0.6, quantile_lines = TRUE,
                        quantile_fun = mean)+
    labs(x= expression("Growth rate day"^"-1"), y = "", title = title)+
    #stat_bin(binwidth = bin_width_c)+
    scale_x_continuous(limits = c(0,11))+
    #scale_y_discrete(expand = expand_scale(mult = c(0.2, 1.5)))+
    scale_fill_manual(values = palette_class_assigned)+
    #scale_y_continuous(limits = c(y1_cl,y2_cl))+
    facet_grid(rows = vars(class_f), scales = 'free', switch = 'y', space = 'free')+
    geom_text(nudge_x = 8.8, nudge_y = 0.3, check_overlap = TRUE, size = 1)+
    theme_ridges(center_axis_labels = TRUE)+ 
    theme(strip.text.x = element_text(size = 0), 
          axis.text.y = element_text(angle = 0,  size = 0), 
          axis.text.x = element_text(size = 3),
          axis.ticks = element_blank(), legend.position = "none", plot.title = element_text(size = 7),
          panel.grid.major = element_blank(), panel.grid.minor = element_blank(), strip.background = element_blank(),
          axis.title.x = element_text(size = 7),
          plot.margin= unit(c(1.6, 0.2, 0.2, 0.2), "lines")) #top, right, bottom, left
  ...
  plot_order <- 
    ...
    filter(n() > x_c) %>%
    ungroup() %>%
    ggplot(aes(y = fct_rev(order_f), x = slope_chosen_days, fill = class_f, label = counts))+
    ...
    geom_density_ridges(aes(fill = class_f), scale = 2, alpha = 0.6, quantile_lines = TRUE,
                        quantile_fun = mean)+
    facet_grid(rows = vars(class_f), scales = 'free', space = 'free')+
    geom_text(nudge_x = 8.8, nudge_y = 0.3,  size = 1, check_overlap = TRUE)+
    labs(x= expression("Growth rate day"^"-1"), y = "", title = 'Order')+
    #stat_bin(binwidth = bin_width_o)+
    scale_x_continuous(limits = c(0,11))+
    ##scale_y_discrete(expand = expand_scale(mult = c(0.2, 1.5)))+ #perquè no es talli per dalt
    scale_fill_manual(values = palette_class_assigned)+
    #scale_y_continuous(limits = c(y1_o,y2_o))+
    theme_ridges(center_axis_labels = TRUE)+ 
    theme(strip.text.x = element_text(size = 0), axis.text.x = element_text(angle = 0, size = 5), 
          ...
          axis.title.x = element_text(size = 7),
          plot.margin= unit(c(1.6, 0.2, 0.2, 0.2), "lines"))
  ...
  plot_family <- 
    ...
    filter(n() > x_o) %>%
    ungroup() %>%
    ggplot(aes(y = fct_rev(family_f), x = slope_chosen_days, fill = class_f, label = counts))+ #interaction( family_f, order_f, sep = '\n')
    # geom_histogram(aes(x=slope_chosen_days, group = family), colour = "#FFDF26", fill = "#FFDF26", #adjust= 0.5, 
    #                # bins = bin_width_f,
    #                binwidth = bin_width_f,
    #                show.legend = FALSE, alpha = 0.2)+
    geom_density_ridges(aes(fill = class_f), scale = 3, alpha = 0.6,
                        quantile_lines = TRUE,
                        quantile_fun = mean)+
    labs(x= expression("Growth rate day"^"-1"), y = "", title = 'Family')+
    #stat_bin(binwidth = bin_width_f)+
    scale_x_continuous(limits = c(0,11))+
    #scale_y_discrete(expand = expansion(mult = c(0, 2)))+
    scale_fill_manual(values = palette_class_assigned)+
    facet_grid(rows = vars(order_f), scales = 'free', space = 'free')+
    geom_text(nudge_x = 8.8, nudge_y = 0.3, check_overlap = TRUE, size = 1)+
    # scale_y_continuous(limits = c(y1_f,y2_f))+
    facet_grid(rows = vars(order_f), scales = 'free', space = 'free', switch = 'y')+
    theme_ridges(center_axis_labels = TRUE)+ 
    theme(strip.text.x = element_text(size = 0), axis.text.x = element_text(angle = 0, size = 5), 
          ...
          plot.title = element_text(size = 7),
          axis.title.x = element_text(size = 7),
          plot.margin= unit(c(1.9, 0.2, 0.2, 0.2), "lines"))
  ...
  plot_asv <- data_filt %>%
    ...
    filter(n() > x_f) %>%
    ungroup() %>%
    ggplot(aes(y = fct_rev(asv_f), x = slope_chosen_days, fill = class_f, label = counts))+
    ...
    geom_text(nudge_x = 8.8, nudge_y = 0.3, check_overlap = TRUE, size = 1)+ #
    theme_ridges(center_axis_labels = TRUE)+ 
    theme(strip.text.x = element_text(size = 5), axis.text.x = element_text(angle = 0, size = 5), legend.position = "none",
